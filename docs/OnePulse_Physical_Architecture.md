...
|-------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Logical Component**               | **Technology**                                                                                                                                                                                                                                           | **Justification**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| Identity, Access & Isolation        | Microsoft Entra ID (authentication); Apigee policies (authorization, rate limiting); Postgres Row-Level Security on Azure Database for PostgreSQL Flexible Server (data isolation)                                                                                                                | Enforced at the database layer, not application code alone — directly satisfies NFR-1's explicit requirement.                                                                                                                                                                                                                                                                                                                                                                                                         |
...
| Report & Evidence Store   | Azure Database for PostgreSQL Flexible Server                                                                        | Relational tables, keyed through the Tenant → Portfolio → Program hierarchy defined in Logical Architecture.                                                                                                                                                                       |
| Approval & Governance Log | Azure Database for PostgreSQL Flexible Server, same instance, append-only table with no update or delete permission granted to the application role | The append-only guarantee is enforced at the database permission level, not just by application discipline — consistent with this project's standing rule of enforcing guarantees in code, never trusting good behavior alone.                                                     |
| Configuration Store       | Azure Database for PostgreSQL Flexible Server, same instance                                                          | Co-located with the other stores; distinguished by access pattern (Section 4.3, Logical Architecture), not by separate infrastructure.                                                                                                                                             |
...
**Revision note (supersedes the original Step 6 choice):** This document
originally specified Neon as the Postgres technology. Neon does not
support Microsoft Entra ID / Managed Identity authentication for the
actual database connection — its Postgres roles are password-only, and
even Neon's Azure Native Integration supports Managed Identity only for
control-plane operations (project/branch management), not the wire
connection itself, confirmed directly against Neon's role-authentication
documentation and Azure Service Connector's supported-auth-methods table
for Neon (which lists connection-string as the *only* supported method
for every client type). This directly conflicted with NFR-2 and this
section's own Managed Identity claim below. Azure Database for PostgreSQL
Flexible Server is substituted for all three Postgres-backed stores
because it natively supports Entra ID token authentication for the
connection itself — the only option that satisfies NFR-2 literally, with
zero static credential at any point, not merely zero credential in
application code.

# 5. Network & Security Topology

Kafka and the core services run inside the AKS cluster's private
network. Temporal Cloud and Azure Database for PostgreSQL Flexible Server
are external managed services reached over the public internet with TLS
— an explicit, accepted trade-off: a
...
separate AKS namespace and an isolated per-environment store on Azure
Database for PostgreSQL Flexible Server — the exact isolation mechanism
(separate server instances versus separate databases on one instance) is
specified in DevOps Setup Section 4, not duplicated here, since Neon's
branch-per-environment model has no direct equivalent on this technology
and the replacement needed its own trade-off review. Azure DevOps
