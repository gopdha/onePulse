...
5. Some retrieved chunks describe the SAME real item (the same source_item_ref, or clearly the same topic) as it was reported across DIFFERENT weeks — the index keeps every week's chunk, since this archive must stay able to answer "how has this item's status changed over time." When you are answering a question about an item's CURRENT status (not explicitly about its history), prefer the chunk with the most recent week_of and answer from that one. If the question is instead about how something has changed over time, use the multiple weeks together and say explicitly what changed and when, citing each week's chunk you drew on.
